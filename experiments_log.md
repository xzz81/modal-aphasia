...
## 2026-06-25 - Tianyang Janus-Pro Faces and Safety Completion

- goal: complete the remaining modal_aphasia Janus-Pro benchmark fine-tuning families on tianyang after the full synthetic concepts stage.
- hypothesis or change: keep the corrected faces effective global batch at 32 and let the guarded safety_unsafe queue launch only after the final faces checkpoint is complete.
- command/config: faces used `/opt/venv/bin/python -m accelerate.commands.launch --config-file configs/faces_janus/accelerate_config.yaml --num_processes 4 -m modal_aphasia.janus.train --setting faces --num-epochs 100 --aux-fraction 0.0 --language-model-only --per-device-train-batch-size 1 --gradient-accumulation-steps 8 --save-strategy no --eval-steps 20 --save-final-model`; safety_unsafe used `/opt/venv/bin/python -m accelerate.commands.launch --config-file configs/safety/accelerate_config.yaml --num_processes 4 -m modal_aphasia.janus.train --setting safety_unsafe --num-epochs 6 --aux-fraction 0.0 --language-model-only --per-device-train-batch-size 1 --gradient-accumulation-steps 1 --save-strategy no --eval-steps 20 --num-train-val-samples 50 --num-aux-val-samples 0 --save-final-model`.
- data/model paths: `data/faces`; `data/safety_images`; `model/Janus-Pro-7B`; final checkpoints `model/finetuned_janus/faces/janus-faces-seed178430-20260624` and `model/finetuned_janus/safety/janus-safety-unsafe-seed178430-20260624`.
- output paths: faces log `runs/janus_faces_queued_20260624.log`; safety log `runs/janus_safety_queued_20260624.log`; concepts checkpoint remains `model/finetuned_janus/concepts/janus-concepts-24perm-noaux-seed178430-20260624`.
- result: faces completed 1900/1900 optimizer steps with `train_runtime=31595.7184`, `train_loss=52.331426002602825`, final `eval_val_loss=0.9198492169380188`, and `Finished training`; safety_unsafe then completed 78/78 optimizer steps with `train_runtime=209.3662`, `train_loss=24.588997865334534`, and `Finished training`. Concepts, faces, and safety_unsafe final checkpoints each contain `config.json`, `model.safetensors.index.json`, and seven safetensors shards, about 28GB per checkpoint.
- interpretation: the requested Janus-Pro fine-tuning coverage now includes three modal_aphasia benchmark families on tianyang: synthetic concepts, faces with original effective batch semantics, and safety_unsafe. The safety_refusal phase is still not included because the current Janus training code implements `safety_unsafe` but not `safety_refusal`.
- next step: run or add Janus-Pro-native evaluation for these checkpoints if result comparison, not only fine-tuning, is required.
