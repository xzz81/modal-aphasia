...
for seed_idx in "${!SEEDS[@]}"; do
    ...
    file_stem_synthetic_concepts="${model_id}_synthetic_concepts_hd"
    uv run -m modal_aphasia.evals.grade_synthetic_images \
        --input "${RESULTS_ROOT}/${file_stem_synthetic_concepts}.jsonl" \
        --output "${RESULTS_ROOT}/${file_stem_synthetic_concepts}_graded.jsonl"
    ...
    file_stem_concepts_description_mc="${model_id}_concepts_description_mc"
    uv run -m modal_aphasia.evals.grade_multiple_choice \
        --input "${RESULTS_ROOT}/${file_stem_concepts_description_mc}.jsonl" \
        --output "${RESULTS_ROOT}/${file_stem_concepts_description_mc}_graded.jsonl" \
        --seed "${seed}"

    # Ablation description
    file_stem_concepts_description_ablation_mc="${model_id}_concepts_description_ablation_mc"
    uv run -m modal_aphasia.evals.grade_multiple_choice \
        --input "${RESULTS_ROOT}/${file_stem_concepts_description_ablation_mc}.jsonl" \
        --output "${RESULTS_ROOT}/${file_stem_concepts_description_ablation_mc}_graded.jsonl" \
        --seed "${seed}"
